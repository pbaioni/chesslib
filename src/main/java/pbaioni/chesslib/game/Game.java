...
import pbaioni.chesslib.move.MoveException;
...
public class Game {
		...
		if (getHalfMoves().size() == 0) {
			...
			for (int i = 0; i < sanArray.length; i++) {
				...
				if (getVariations() != null) {
					...
					if (var != null) {
						variantIndex = translateVariation(sb, var, -1, variantIndex, index, moveCounter, lastSize);
						if (index % 2 != 0) {
							sb.append(" " + moveCounter + "...");
						}
					}
				}
				if (i < sanArray.length - 1 && index % 2 == 0 && index >= 2) {
					moveCounter++;
	...
	private int translateVariation(StringBuilder sb, MoveList variation, int parent, int variantIndex, int index,
			int moveCounter, int lastSize) throws MoveConversionException {
		...
		if (variation != null) {
			...
			for (i = 0; i < sanArray.length; i++) {
				...
				if (child != null) {
					...
					variantIndex = translateVariation(sb, child, variantIndexOld, variantIndex, idx, mc, lastSize);
				}
	...
	public void loadMoveText() throws Exception {
		...
		for (String token : text.split(" ")) {
			...
			if (!(onLineCommentBlock || onCommentBlock) && token.contains("...")) {
			...
			if (token.equals("{") && !(onLineCommentBlock || onCommentBlock)) {
				...
				comment = null;
				continue;
			} else if (token.equals("(") && !(onCommentBlock) || onLineCommentBlock) {
				onVariationBlock = true;
				variation.add(new RTextEntry(variantIndex));
				continue;
			} else if (token.equals(")") && onVariationBlock && !(onCommentBlock) || onLineCommentBlock) {
					...
					try {
						...
						if (last != null) {
							throw new PgnException("Error while reading variation: "
									+ getMovesAt(currentLine.toString(), last.index) + " - " + last.text.toString(), e);
						} else {
	...
	public void gotoMove(final MoveList moves, int index) throws MoveException {
		...
		if (getBoard() != null && index >= 0 && index < moves.size()) {
